-- Settle a membership payment and extend the entitlement in one transaction.
-- The advisory lock serializes payments for one LINE account, so two callbacks
-- arriving concurrently cannot both calculate from the same old expiry date.
...
begin
  ...
  if p_target_status = 'paid' then
    ...
    -- A hash collision can only serialize unrelated accounts; it cannot grant
    -- an entitlement to the wrong account.
